class CeapTseController < ApplicationController
  def index
    ...
    encoded_url = URI.encode('http://localhost:2481/query/CotasParlamentares/sql/MATCH {class:Parlamentar, as:p} -RealizaTransacao-> {class:Transacao, as:t} -FornecidaPor-> {class:EmpresaFornecedora, as:e}, {as:e} -RealizaTransacao-> {class:Transacao, as:t2} -FornecidaPara-> {as:p} RETURN p.TxNomeParlamentar limit 500')
