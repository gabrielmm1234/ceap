$( document ).ready(function() {
  ...
  $.ajax({
    type: "GET",
    url: "/ceap_fraud_deputies_relatives",
    success: function(data){
      var s = new sigma('container-graph-family');
      var label = undefined
      var color = undefined
      var x = undefined
      var y = undefined
      ...
      data["result"].forEach(function(item, index) {
        if (item["@class"] == "Transacao") {
          ...
          x = Math.random()
          y = Math.random() * 40
        } else if(item["@class"] == "EmpresaFornecedora") {
          color = '#ec5148'
          label = item['TxtFornecedor']
          x = Math.random() + 50
          y = Math.random() * 40
      ...
      var deputies = data["result"].filter(function(item) { return item["@class"] == "Parlamentar" })
      var companies = data["result"].filter(function(item) { return item["@class"] == "EmpresaFornecedora" })
      var persons = data["result"].filter(function(item) { return item["@class"] == "Pessoa" })
      ...
      persons.forEach(function(item) {
        deputy = deputies.filter(function(deputy_item) {
          return deputy_item["out_Parente_De"].includes(item["in_Parente_De"][0])
        })
